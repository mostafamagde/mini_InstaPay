...
class NotificationsRepo {
  ...
  Future<List<NotificationModel>> getAllNotifications() async {
    final response = await _apiManger.get(ApiConstants.getAllNotifications,
  ...
  Future<void> readNotifications(String notificationId) async {
    await _apiManger.patch(ApiConstants.readNotifications + notificationId,
        headers: {"token": UserModel.instance.token});
  }

  Future<void> rejectRequest({required String transactionId}) async {
    await _apiManger.post(ApiConstants.rejectReceive + transactionId, {},
        headers: {"token": UserModel.instance.token});
  }
  ...
  Future<void> acceptRequest(
    ...
    await _apiManger.post(ApiConstants.confirmReceive + transactionId, body,
        headers: {"token": UserModel.instance.token});
  }

  Future<void> rejectRefund({required String transactionId}) async {
    await _apiManger.post(
        ApiConstants.rejectRefund, {"transactionId": transactionId},
        headers: {"token": UserModel.instance.token});
  }

  Future<void> acceptRefund({required String transactionId}) async {
    await _apiManger.post(
        ApiConstants.acceptRefund, {"transactionId": transactionId},
        headers: {"token": UserModel.instance.token});
  }
}
